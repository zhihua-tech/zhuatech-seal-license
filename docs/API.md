...
| POST | `/domain/decision` | 执行企业印章证照与资质管理系统专属领域规则 |
...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `applicationNo` | String | 用印申请编号 |
| `approvalLevelsPassed` | int | 已通过审批级数 |
| `requiredApprovalLevels` | int | 要求审批级数 |
| `useCount` | int | 用印份数 |
| `sealActive` | boolean | 印章状态有效 |
| `licenseValid` | boolean | 关联证照有效 |
| `requesterAuthorized` | boolean | 申请人具备权限 |
| `returned` | boolean | 借用原件已归还 |
| `evidenceAttached` | boolean | 用印影像已归档 |
| `locationVerified` | boolean | 使用地点已核验 |
